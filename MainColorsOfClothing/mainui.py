...
class Ui_MainWindow(object):
    ...
    def process_image(self):
        ...
        if self.image_path:
            ...
            HSV.plot_histograms(hsv_image, foreground_mask)
            ...
            # 尝试显示main_color.png在label_4中
            color_image_path = "main_color.png"
            if os.path.exists(color_image_path):
                pixmap_color = QtGui.QPixmap(color_image_path)
                if not pixmap_color.isNull():
                    self.label_4.setPixmap(pixmap_color.scaled(self.label_4.size(), QtCore.Qt.KeepAspectRatio))
                else:
                    print(f"Failed to load image: {color_image_path}")
            else:
                print(f"Image not found: {color_image_path}")
            ...
            QtWidgets.QMessageBox.warning(None, "警告", "请先上传图片")
